		      ...
		      int ws_x, int ws_y)
{
	 ...
	 */
	g_signal_connect(g->button1, "Run", G_CALLBACK(pause_cb), NULL);
	g_signal_connect(g->button2, "Step", G_CALLBACK(step_cb), NULL);
	g_signal_connect(g->button3, "Quit", G_CALLBACK(gtk_main_quit), NULL);
	 ...
	 */
	g-> cell_size -> x = WINDOW_X + ws_y;
	g-> cell_size -> y = WINDOW_Y + ws_x;	
